...
ssp4_2030_global <- rast("data/SSP4_RCP60/global_PFT_SSP4_RCP60_2030.tif")   #2030
ssp4_2040_global <- rast("data/SSP4_RCP60/global_PFT_SSP4_RCP60_2040.tif")   #2040
ssp4_2050_global <- rast("data/SSP4_RCP60/global_PFT_SSP4_RCP60_2050.tif")   #2050
...
Next, reclassify layers each projected LULC layer to isolate urban pixels (value 19).
```{r}
...
Now that we've isolated Brazilian cropland, we want to determine where current cropland is projected to become "abandoned" in the future. This will done with a simple raster calculation, subtracting the future cropland raster from the current cropland raster. Values of 0 indicate land that either remained cropland (1-1) or remained non-cropland (0-0). Values of -1 indicate new future cropland (0-1), while values of 1 indicate abandoned cropland (1-0). 
...
```
These layers may be useful for future study into the rates of LULC transition over time between cropland and urban classification. For this analysis, we want to isolate just pixels of non-urbanized cropland abandonment. In this next step, we'll reclassify the abandonment layers to only return pixels of non-urbanized abandonment (for ease of next analysis steps)
...
Then, values for an SSP will be summed to return one spatial raster indicating every pixel of cropland was projected to be abandoned *at some point* between 2020-2050.
...
abandoned_ssp1 <- ab_ssp1_2020u + ab_ssp1_2030u + ab_ssp1_2040u + ab_ssp1_2050u
abandoned_ssp1 <- classify(abandoned_ssp1, mat)

# add SSP2 layers and reclassify
abandoned_ssp2 <- ab_ssp2_2020u + ab_ssp2_2030u + ab_ssp2_2040u + ab_ssp2_2050u
abandoned_ssp2 <- classify(abandoned_ssp2, mat)

# add SSP3 layers and reclassify
abandoned_ssp3 <- ab_ssp3_2020u + ab_ssp3_2030u + ab_ssp3_2040u + ab_ssp3_2050u
abandoned_ssp3 <- classify(abandoned_ssp3, mat)

# add SSP4 layers and reclassify
abandoned_ssp4 <- ab_ssp4_2020u + ab_ssp4_2030u + ab_ssp4_2040u + ab_ssp4_2050u
abandoned_ssp4 <- classify(abandoned_ssp4, mat)

# add SSP5 layers and reclassify
abandoned_ssp5 <- ab_ssp5_2020u + ab_ssp5_2030u + ab_ssp5_2040u + ab_ssp5_2050u
abandoned_ssp5 <- classify(abandoned_ssp5, mat)
```
